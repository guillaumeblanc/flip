...
void RendererImpl::EndImDraw(sg_image _image, bool _linear) {
  resources_->im_drawer.End(_image, _linear);
}
  ...
  // Opaque grid lines
  {
    for (auto& transform : _transforms) {
      auto drawer = ImDraw{
          *this,
          transform,
          {.type = SG_PRIMITIVETYPE_LINES, .cull_mode = SG_CULLMODE_NONE}};

      drawer.color(.5f, .5f, .5f, 1.f);
